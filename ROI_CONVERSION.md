...
`scripts/netlify-build.sh` now runs, in order:

1. `build/build.py`
2. `build/postprocess_audit_fixes_v2.py`
3. `build/postprocess_roi_conversion.py`
4. Netlify publishes `site/` only after all three stages pass.

The ROI postprocessor adds and validates:
...
Do not remove this postprocessor unless its behavior has been moved upstream and the same built-output invariants remain independently tested.